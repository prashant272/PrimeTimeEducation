...
import { sendNominationConfirmation, sendNominationCredentialsEmail, sendNominationAndCredentialsEmail } from "../services/emailService.js";
...
// Create a nomination (handles both logged-in and guest users)
router.post("/", optionalAuthenticate, upload.single("pdf"), async (req, res) => {
  try {
    ...
    // If not authenticated, ensure we have an email and identify/create user
    if (!userId) {
      const contactEmail = payload.contactEmail || payload.email;
      const contactName = payload.contactName || payload.nomineeName || "User";

      if (!contactEmail) {
        return res.status(400).json({ message: "Contact email is required for unauthenticated submissions" });
      }
      ...
      if (!existingUser) {
        ...
        const passwordHash = await bcrypt.hash(autoGeneratedPassword, salt);

        existingUser = await User.create({
          name: contactName,
          email: contactEmail.toLowerCase(),
          passwordHash,
          isVerified: true, // We auto-verify as they are submitting a nomination
        });
    ...
    // Send email(s) asynchronously
    if (credentialsSent) {
      sendNominationAndCredentialsEmail(userEmail, userName, autoGeneratedPassword).catch(err =>
        console.error("Async combined email error:", err)
      );
    } else {
      sendNominationConfirmation(userEmail, userName).catch(err =>
    ...
    return res.status(201).json({
      ...
      message: credentialsSent
        ? "Your account is created successfully. Email with ID and password has been sent."
